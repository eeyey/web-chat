    ...
    async function startCapture() {
      ...
      if (call.type === CallType.CREATED) {
        connection.createOffer().then((description) => {
          ...
          socket.emit('RTCConnect', { user_id: call.user_id, description });
        });

        socket.on('RTCConnect', ({ description }) => {
          connection.setRemoteDescription(description); // Нужно убедиться, что соединение стабильно.
        });
      } else {
  ...
  return (
    <div className="call-wrapper">
      <div className="call-window">
        <CallMenu stream={mediaStream} video={true} />
